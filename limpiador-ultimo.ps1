Write-Host "ELIMINACION FINAL DE MATH.RANDOM" -ForegroundColor Red
...
foreach ($file in $files) {
    if (Test-Path $file) {
        ...
        if ($content -ne $originalContent) {
            ...
            Write-Host "✅ $file : $replacements reemplazos" -ForegroundColor Green
        } else {
            Write-Host "⚠️ $file : Sin cambios" -ForegroundColor Yellow
        }
    } else {
...
Write-Host "ELIMINACION COMPLETA DE MATH.RANDOM" -ForegroundColor Green
