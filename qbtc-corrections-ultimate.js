...
function calculateQBTCEntanglement_CORRECTED(sectorData) {
    ...
    const signalFactor = Math.sin(Math.max(1, signals) / QBTC_QUANTUM_CONSTANTS.LAMBDA_7919) * Math.cos(confidence / QBTC_QUANTUM_CONSTANTS.PHI_GOLDEN);
    const strengthFactor = Math.tanh(strength / 100) * QBTC_QUANTUM_CONSTANTS.LAMBDA_7919;
    const volumeFactor = Math.log(Math.max(1, volume)) / Math.log(1000000) * QBTC_QUANTUM_CONSTANTS.PHI_GOLDEN;
...
function calculateWhaleFlow_CORRECTED(sectorData) {
    ...
    const directionFactor = (whaleFactor + (QBTC_QUANTUM_CONSTANTS.LAMBDA_7919 % 1)) / 2;
...
function calculateInstitutionalFlow_CORRECTED(sectorData) {
    ...
    // Dirección basada en múltiples factores
    const directionFactor = (institutionalFactor + (QBTC_QUANTUM_CONSTANTS.PHI_GOLDEN % 1)) / 2;
